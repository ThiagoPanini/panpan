...
function showWithoutAnimation(): void {
  ...
  gsap.set('.ai-usage-header, .ai-card, .ai-tab-bar', { opacity: 1, y: 0 });
}

/** Scroll-triggered entrance animations and tab interactions for AI Usage. */
function initAIUsageAnimations(): void {
  ...
  const tabBar = section.querySelector<HTMLElement>('.ai-tab-bar');
  const tabs = section.querySelectorAll<HTMLButtonElement>('.ai-tab');
  const panels = section.querySelectorAll<HTMLElement>('.ai-panel');
  const indicator = section.querySelector<HTMLElement>('.ai-tab-indicator');

  // ── Scroll entrance: header ──
  if (header) {
    gsap.to(header, {
      opacity: 1,
      y: 0,
  ...
  if (tabBar) {
    gsap.to(tabBar, {
      ...
      duration: 0.7,
      ease: 'power3.out',
      scrollTrigger: { trigger: tabBar, start: 'top 88%', once: true },
    });
  }

  // ── Scroll entrance: stagger cards in active panel ──
  function animateCardsIn(panel: HTMLElement): void {
    const cards = panel.querySelectorAll<HTMLElement>('.ai-card');
  ...
  // Initial entrance for the active panel (scroll-triggered)
  const activePanel = section.querySelector<HTMLElement>('.ai-panel--active');
  if (activePanel) {
    const cards = activePanel.querySelectorAll<HTMLElement>('.ai-card');
    if (cards.length > 0) {
      gsap.to(cards, {
        opacity: 1,
        y: 0,
  ...
  // ── Position the sliding indicator on the active tab ──
  function positionIndicator(tab: HTMLButtonElement): void {
    if (!indicator || !tabBar) return;
    const barRect = tabBar.getBoundingClientRect();
    const tabRect = tab.getBoundingClientRect();
    // Account for the 4px padding on the bar
    indicator.style.width = `${tabRect.width}px`;
    indicator.style.transform = `translateX(${tabRect.left - barRect.left - 4}px)`;
  }

  // Set initial indicator position
  const activeTab = section.querySelector<HTMLButtonElement>('.ai-tab--active');
  if (activeTab) positionIndicator(activeTab);

  // ── Tab switching ──
  tabs.forEach((tab) => {
    tab.addEventListener('click', () => {
      if (tab.classList.contains('ai-tab--active')) return;
      ...
      panels.forEach((panel) => {
        if (panel.id === targetId) {
          // Fade out old, show new
          const oldPanel = section.querySelector<HTMLElement>('.ai-panel--active');
          if (oldPanel && oldPanel !== panel) {
            gsap.to(oldPanel, {
              ...
              ease: 'power2.in',
              onComplete: () => {
                oldPanel.classList.remove('ai-panel--active');
                oldPanel.hidden = true;
                oldPanel.style.opacity = '';

                panel.hidden = false;
                panel.classList.add('ai-panel--active');
  ...
  window.addEventListener('resize', () => {
    const current = section.querySelector<HTMLButtonElement>('.ai-tab--active');
